...
const addCart = async (req, res) => {
 try{
  ...
  const isFoodExists = await Cart.findOne({ where: { foodId, cart_code: findCartCode?.cart_code } });
...
const updateCart = async (req, res) => {
  try{
    ...
    const updateQuantity = await Cart.update({ no_of_item }, {where: { id: cartId }})
...
const getCart = async (req, res) => {
  try{
    ...
    const findCartCode = await User.findOne({ where: { id: userId }, attributes: [`cart_code`] });

    if(findCartCode){
      const getCartItems = await Cart.findAll({ where: { cart_code: findCartCode?.cart_code } , include: [Food] });
      
      if(getCartItems?.length){
        return res.status(200).send({ message: "Successfully fetched data", data: getCartItems });
      }else{
...
const deleteCartItem = async (req, res) => {
  try{
    ...
    const deleteFood = await Cart.destroy({ where: { foodId, userId, id: cartId } });
